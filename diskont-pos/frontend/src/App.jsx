...
import { ShoppingCart, Barcode, Trash2, CheckCircle2, AlertTriangle, Search, Package } from 'lucide-react';
      ...
      <div className="w-2/3 flex flex-col border-r border-slate-800 p-6">
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-emerald-500 transition"
          />
        </div>

        {/* Product Catalog Grid */}
        <div className="grid grid-cols-3 gap-4 overflow-y-auto pr-2 flex-1 auto-rows-max">
          {filteredProducts.map((p) => (
            <div
              key={p.id}
              onClick={() => addToCart(p, 1)}
              className="bg-slate-900/90 p-4 rounded-xl border border-slate-800 hover:border-emerald-500/50 hover:bg-slate-800/80 cursor-pointer transition flex flex-col justify-between shadow-lg group"
            >
              <div>
                ...
                <h3 className="font-semibold text-base text-white mt-2 group-hover:text-emerald-300 transition">{p.name}</h3>
              </div>
              <div className="mt-4 pt-3 border-t border-slate-800/80 flex justify-between items-end">
                <div>
                  <span className="text-emerald-400 font-bold text-lg">{p.unit_price} RSD</span>
                  {p.pack_price && (
                    <p className="text-xs text-slate-400 font-medium">Pack ({p.units_in_pack}x): {p.pack_price} RSD</p>
                  )}
                </div>
                <span className={`text-[11px] font-semibold px-2 py-1 rounded-md ${p.stock_units < p.min_stock_units ? 'bg-red-950 text-red-400 border border-red-800' : 'bg-slate-800 text-slate-400 border border-slate-700'}`}>
                  Stock: {p.stock_units}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3 overflow-y-auto max-h-[55vh] pr-1">
              {cart.length === 0 ? (
                <div className="text-center py-12 text-slate-500">
                  ...
                  <p className="text-sm">Cart is empty. Scan barcode or click a product to add.</p>
                </div>
              ) : (
                cart.map((item) => (
                  <div key={item.product.id} className="flex justify-between items-center bg-slate-950 p-3.5 rounded-xl border border-slate-800/80">
                    <div>
                      <h4 className="font-semibold text-white text-sm">{item.product.name}</h4>
                      <p className="text-xs text-emerald-400 font-medium mt-0.5">{item.product.unit_price} RSD / unit</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="flex items-center bg-slate-800 rounded-lg border border-slate-700">
                        <input
                          ...
                          className="w-12 text-center bg-transparent text-white font-bold text-sm py-1 focus:outline-none"
                        />
                      </div>
